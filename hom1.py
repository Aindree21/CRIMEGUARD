...
def basicSetup(pageNo):
    ...
    right_frame = tk.LabelFrame(content, bg="#202d42", font="Arial 20 ")
    right_frame.grid(row=0, column=1, sticky="nsew", padx=20, pady=20)
...
def getPage4():
    ...
    basicSetup(1)
    heading.configure(text="Crime Twitter Analysis")
    
    # Load and display the logo
    logo = tk.PhotoImage(file="analysis1.PNG")
    tk.Label(right_frame, image=logo, bg="#202d42").pack()
    # Keep a reference to the logo to prevent it from being garbage collected
    heading.image = logo
    print(logo)
    ...
    # Define and initialize menu_var and opt_menu before creating the button
    menu_var = tk.StringVar()
    opt_menu = tk.OptionMenu(left_frame, menu_var,"option")
    opt_menu.pack()

    tk.Button(btn_grid, text="Twitter Analysis", 
              command=lambda: selectMultiImage(opt_menu, menu_var), 
              font="Arial 15 bold", bg="#2196f3", fg="white", pady=5, bd=0, 
              highlightthickness=0).grid(row=0, column=0, padx=20, pady=22)
...
tk.Button(btn_frame, text=" Analysis", command=getPage4)
